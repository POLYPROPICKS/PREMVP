...
function reservation(
  planningIdentity?: { condition_id: string; token_id: string; side: string },
  ...
  return {
    ...
    diagnostics: {
      contract_a_stage: "PLANNING", source_lineage: { generated_signal_pair_id: "pair-anchor" },
      ...(planningIdentity
        ? {
            planning_final_identity_evidence: {
              condition_id: planningIdentity.condition_id,
              token_id: planningIdentity.token_id,
              side: planningIdentity.side,
              market_slug: null,
...
test("minimal due path selects the validated Planning final identity -- NOT the highest score, NOT the lexicographically-smallest identity -- and creates one Queue row", async () => {
  ...
  // pinned as the Planning-committed identity, so it must win regardless.
  ...
  assert.equal(queue[0].condition_id, "cond-low", "the Planning-pinned identity wins despite its lower score and its non-minimal lexical order");
  assert.equal(queue[0].score, 12);
  assert.equal((queue[0].diagnostics as Record<string, unknown>).selected_signal_pair_id, "pair-low");
...
test("PostgreSQL-equivalent identical idempotency conflict returns ALREADY_QUEUED", async () => {
  const r = reservation({ condition_id: "cond-a", token_id: "token-cond-a", side: "YES" });
...
test("persisted lifecycle parity: one frozen UTC asOf gives the same active/due set to audit and real entry", async () => {
  const pinned = { condition_id: "cond-high", token_id: "token-cond-high", side: "YES" };
  const active = reservation(pinned);
  const terminal = { ...reservation(pinned), id: "reservation-terminal", status: "SKIPPED" as const, selection_reason: "EXACT_PROVIDER_EVENT_IDENTITY_MISSING" };
  const expired = { ...reservation(pinned), id: "reservation-expired", game_start_iso: "2026-08-06T17:02:00.000Z", event_start_iso: "2026-08-06T17:02:00.000Z" };
