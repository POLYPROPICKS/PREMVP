...
test("forward funnel preserves Contract A conservation and deterministic reservation comparison", () => {
  ...
  const report = summarizeForwardFunnel({ rows: [row("2026-08-06T21:00:00.000Z")], decisions: [accepted, rejected], reservationPhysicalEventIds: ["event-a"], planningSelected: 1, startMs: start, endMs: end });
  assert.equal(report.contract_a_input, report.contract_a_accepted + report.contract_a_rejected);
