...
test("C3: default CONTUR3_CURRENT candidates (no selector_id) claim no Contract A authority, but still persist their own structured planning identity", async () => {
  const plan = await buildReservationPlan(ANCHOR_NOW_MS, {
    fetchCandidates: async () => ({ candidates: [baseCandidate()] }),
  });
  const r = plan.reservations[0];
  ...
  // R0F stage 1: every reservation stores the structured planning identity it
  // was planned against -- no reservation may exist without one. The executable
  // identity belongs to the final stage and is absent here by design.
  assert.equal(r.diagnostics.planning_stage, "PLANNING_EVENT_IDENTITY");
  assert.equal(r.diagnostics.planning_event_key, "slug:fifwc-esp-arg-2026-07-19");
  assert.equal(r.diagnostics.planning_game_start_iso, KICKOFF_ISO);
...
test("C5: Contract A planning reserves an event five hours away, persisting a structured planning identity and no false final authority", async () => {
  ...
  assert.equal(plan.reservations[0].diagnostics.planning_stage, "PLANNING_EVENT_IDENTITY");
  assert.equal(plan.reservations[0].diagnostics.planning_event_key, "slug:fifwc-esp-arg-2026-07-19");
  // The T-90 decision does not exist five hours out: planning must not claim it.
