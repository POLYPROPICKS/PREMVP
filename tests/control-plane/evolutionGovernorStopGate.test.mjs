...
function fakeGit({ branch = 'claude/evolution-lineage', hasOriginMain = true, porcelain = '', diffFiles = [], remoteDiffFiles = diffFiles, fetchError = null } = {}) {
  const calls = [];
  let fetched = false;
  const runGit = (args) => {
    ...
    if (args[0] === 'rev-parse' && args.includes('FETCH_HEAD')) return fetched ? 'fresh-main-sha\n' : '';
    if (args[0] === 'rev-parse' && args.includes('fresh-main-sha')) return fetched ? 'fresh-main-sha\n' : '';
    ...
    if (args[0] === 'diff' && args.includes('--name-only')) return `${(args.some((arg) => arg.startsWith('fresh-main-sha...')) ? remoteDiffFiles : diffFiles).join('\n')}\n`;
    return '';
  };
  runGit.calls = calls;
  runGit.setRemoteDiffFiles = (next) => { remoteDiffFiles = next; };
  return runGit;
}
...
test('TERMINAL: cloud merge sequence refreshes authority and avoids a second canonicalization cycle', () => {
  const runGit = fakeGit({ diffFiles: [CYCLE_JSON, CYCLE_REPORT], remoteDiffFiles: [CYCLE_JSON, CYCLE_REPORT] });
  const first = evaluateStopGate({ runGit, runAdmit: admitOk({ cycles: ['2026-08-29__real'], governor_results: [] }) });
  assert.equal(first.decision, 'BLOCK', 'before the remote merge, the valid lineage must block Stop');
  assert.match(first.message, /control-plane:evolution:canonicalize/);

  // GitHub merges the lineage after the first Stop gate; cached origin/main is intentionally
  // still old until this next Stop evaluation fetches remote main into FETCH_HEAD.
  runGit.setRemoteDiffFiles([]);
  const second = evaluateStopGate({ runGit, runAdmit: admitNeverCalled() });
  assert.equal(second.decision, 'ALLOW');
  assert.equal(second.reason, 'CASE_4_CANONICAL_ON_FRESH_REMOTE_MAIN');
  assert.equal(second.message, '', 'a canonical lineage must not emit another canonicalization instruction');
  assert.ok(runGit.calls.filter((call) => call === 'fetch --no-tags origin refs/heads/main').length >= 2);
});
...
test('TERMINAL: the hook delegates the admission verdict to premvp.command.evolution_canonicalize.v1', () => {
  ...
  assert.equal(seen[0].base, 'fresh-main-sha');
...
test('TERMINAL: inability to refresh canonical remote authority fails closed causally', () => {
  const runGit = fakeGit({ diffFiles: [CYCLE_JSON, CYCLE_REPORT], fetchError: 'remote: temporary transport failure' });
  const verdict = evaluateStopGate({ runGit, runAdmit: admitNeverCalled() });
  assert.equal(verdict.decision, 'BLOCK');
  assert.equal(verdict.reason, 'CASE_5_CANONICAL_AUTHORITY_REFRESH_FAILED');
  assert.match(verdict.message, /temporary transport failure/);
  assert.match(verdict.message, /cached origin\/main/);
});
...
test('refreshCanonicalMainAuthority observes remote main only through FETCH_HEAD', () => {
  const runGit = fakeGit();
  const authority = refreshCanonicalMainAuthority(runGit);
  assert.deepEqual(authority, { ok: true, base: 'fresh-main-sha', sha: 'fresh-main-sha' });
