...
// Extended Historical Decomposition CLI (Phase 4A.2 / A1).
//
// Reads ONLY local files (raw corpus export + classifier registry), builds
// the deterministic extended decomposition via the pure lib module, and
...
const DEFAULT_OUTPUT_DIR = path.join("modeling", "local_exports", "extended_historical_decomposition");

/** Default analysis set: the exported canonical 12-model scorecard order -- never a second hand-maintained list. */
export const DEFAULT_DECOMPOSITION_VARIANTS: readonly string[] = [...SCORECARD_MODEL_ORDER];

const JSON_FILENAME = "extended_historical_decomposition.json";
const MANIFEST_FILENAME = "extended_historical_decomposition_manifest.json";
const HTML_FILENAME = "extended_historical_decomposition_summary.html";

export type ExtendedDecompositionCliMode = "dry-run" | "write";

export interface ExtendedDecompositionArgs {
  mode: ExtendedDecompositionCliMode;
  input: string;
  classifier: string;
  variants: string[];
  outputDir: string;
...
export function parseExtendedDecompositionArgs(argv: string[]): ExtendedDecompositionArgs {
  ...
  if (variants.length === 0) variants = [...DEFAULT_DECOMPOSITION_VARIANTS];
...
export function runExtendedHistoricalDecompositionCli(
  argv: string[],
  log: (msg: string) => void = (m) => process.stderr.write(m),
): number {
  let args: ExtendedDecompositionArgs;
  try {
    args = parseExtendedDecompositionArgs(argv);
  } catch (error) {
  ...
  try {
    ...
    const classifier: ExecutableFunnelClassifier =
      args.classifier === DEFAULT_CLASSIFIER ? loadExecutableFunnelClassifier() : readJson<ExecutableFunnelClassifier>(args.classifier, "classifier");

    const result = buildExtendedHistoricalDecomposition({
      rawRows,
      classifier,
    ...
    const jsonString = serializeExtendedDecompositionJson(result);
    const htmlString = renderExtendedDecompositionSummaryHtml(result);
    const manifest = buildExtendedDecompositionManifest(result, jsonString, htmlString);
    ...
    log(`Wrote extended historical decomposition artifacts to ${args.outputDir}\n`);
    return 0;
  } catch (error) {
    log(`Error: ${error instanceof Error ? error.message : "unknown error"}\n`);
    return 1;
...
if (require.main === module) {
  process.exit(runExtendedHistoricalDecompositionCli(process.argv.slice(2)));
}
