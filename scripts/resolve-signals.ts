...
async function reconcilePendingExecutionSettlements(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  supabase: any,
  writeMode: boolean,
...
async function main() {
  ...
  // The existing resolver cron is the automatic settlement trigger. This
  // bounded sweep is independent of the generic signal backlog, so a signal
  // resolved in an earlier run is still reconciled on the next invocation.
  await reconcilePendingExecutionSettlements(supabase, WRITE_MODE);
