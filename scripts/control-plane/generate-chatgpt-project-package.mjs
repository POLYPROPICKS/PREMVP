...
mission display routing metadata: MODEL, MODEL_LEVEL, GO_COST_CLASS ($60_SAFE / $30_BOUNDED /
$15_SPECIALIST / NON_GO / UNKNOWN), WHY_THIS_MODEL, SESSION, COST_CLASS (CHEAP / STANDARD /
EXPENSIVE), CONTEXT_STRATEGY (REUSE / TARGETED_DISCOVERY / FRESH_BOUNDED), REPOSITORY_READ_BUDGET,
DATA_STRATEGY (NONE / AGGREGATE_FIRST / ROW_LEVEL_REQUIRED), RAW_ROW_BUDGET and VERIFICATION_BUDGET.
For $15_SPECIALIST also require EXPLICIT_COST_JUSTIFICATION. These are presentation metadata,
not Mission Contract core sections, and do not repeat this protocol inside each Mission Contract.

MODEL ECONOMICS
Route by mission, not by prompt turn. MODEL SWITCHING IS NEVER A QUOTA-RESET STRATEGY.
Default READ/SQL/lineage/logs → runtime-confirmed GLM-5.3 Flash family or safer capable
$60_SAFE equivalent. Default implementation → runtime-confirmed Kimi K2.7 Code family or safer
capable equivalent. Mechanical work → proven $60_SAFE low-cost model. Long-context synthesis
→ Qwen3.7 Plus only when useful. $15_SPECIALIST models require EXPLICIT_COST_JUSTIFICATION and
are never the default for exploration, SQL/data, routine diagnosis, implementation, test loops,
logs, mechanical work or broad review. Runtime availability and current provider evidence beat
static tables; never silently upgrade $60_SAFE → $15_SPECIALIST. PREMVP frontier → Claude Code/
Architect; Ireland → Codex only.
