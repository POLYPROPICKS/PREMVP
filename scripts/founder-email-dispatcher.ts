...
function runTrustedMorning(recipient: string, opts: { dryRun: boolean }): void {
...
async function main() {
  ...
  const summary = { mode: modeArg, minskDate, minskMinutes, recipient, dryRun };
  console.log(JSON.stringify(summary, null, 2));

  if (modeArg === "auto") {
    if (inWindow(minskMinutes, 8 * 60 + 55, 9 * 60 + 10)) {
      runTrustedMorning(recipient, { dryRun });
      return;
    }
