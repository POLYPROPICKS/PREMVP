...
// production primary_evidence_outbox -> research_evidence_page_v5() RPC (max 500
...
export async function syncResearchEvidencePage(
  ...
  while (pages < maxPages) {
    ...
    const { data, error } = await source.rpc("research_evidence_page_v5", args);
    if (error) throw new Error(`RESEARCH_CLONE_SOURCE_READ_research_evidence_page_v5:${safeError(error)}`);
