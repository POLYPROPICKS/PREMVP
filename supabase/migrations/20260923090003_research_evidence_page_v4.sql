...
-- Additive v3-compatible narrow projection. v1/v2/v3 remain untouched.
CREATE OR REPLACE FUNCTION public.research_evidence_page_v4(
...
BEGIN
  IF p_until IS NULL THEN
    RAISE EXCEPTION 'research_evidence_page_v4 requires an explicit p_until upper time bound';
  END IF;
...
REVOKE ALL ON FUNCTION public.research_evidence_page_v4(timestamptz, uuid, uuid, timestamptz, integer)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.research_evidence_page_v4(timestamptz, uuid, uuid, timestamptz, integer)
  TO service_role;

COMMENT ON FUNCTION public.research_evidence_page_v4(timestamptz, uuid, uuid, timestamptz, integer) IS
  'Read-only bounded narrow v4 projection of primary_evidence_outbox. Adds only verbatim providerEventContext eventTitle and marketQuestion to the v3 item-level cursor contract.';
