...
-- BOTH_SIDE_EVIDENCE_RELEASE_V1: raise the primary evidence outbox row cap.
--
-- The prior cap (300) predates lib/feed/buildLandingCards.ts
-- fanOutAuthorizedTwoSidedOutcomeCandidates, which can independently score
-- BOTH sides of an authorized binary market instead of collapsing to one.
-- The bounded scorer-input population per producer cycle is
-- PRIMARY_SCORER_PROVEN_CAPACITY (254, lib/feed/buildLandingCards.ts) sampled
-- candidate markets; under full two-sided fan-out each can yield at most 2
-- independently scored token identities, so the maximum bounded primary
-- evidence population per observation is 254 * 2 = 508. This migration only
-- widens the existing finite guard to that derived bound; it does not remove
-- the guard or make it unbounded.
...
ALTER TABLE public.primary_evidence_outbox
  ...
  CHECK (evidence_row_count BETWEEN 1 AND 508);
...
BEGIN
  ...
  IF row_n < 1 OR row_n > 508 THEN
    RAISE EXCEPTION 'primary observation row count must be between 1 and 508';
  END IF;
  IF EXISTS (
