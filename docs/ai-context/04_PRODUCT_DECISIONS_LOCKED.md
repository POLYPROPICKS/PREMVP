...
## CURRENT ROADMAP OVERRIDE — 2026-05-21 (REFRESHED)

> Source of truth: git log + source files. Verified 2026-05-21.
> Prior 2026-05-14/15 override blocks are superseded by this entry.

- Current branch: `main`
- Current HEAD: `264500d` Deploy: force Next.js standalone runtime
- Signal Confidence scoring: on main ✅
- Market Return / American odds: on main ✅ (visual acceptance status UNKNOWN — not recently verified)
- Whop payment integration: SHIPPED (on main) — production verification pending
- Auth (magic link + session): SHIPPED (on main) — production verification pending
- Premium page `/premium`: SHIPPED (on main)
- Signal resolver: SHIPPED (on main)
- Upcoming pairs: SHIPPED (on main, upcomingPairs in API response)
- Production status: NEEDS VERIFICATION — Railway RAILPACK/Caddy issue; manual fix required in Railway Dashboard
- MarketSourceCarousel evidence-stack UI: ON HOLD (until Proof of Results card ships)
- Proof of Results card: Claude Design phase pending — design pack ready
- filterTags one-card-across-filters bug: DEFERRED
- Locked decisions still active: no fake ML, no Win Probability label, no guaranteed profit claims, no independent MarketSourceCarousel browsing
...
## 7. Payment / Auth Decisions (Added 2026-05-21)

### **DECISION: Whop is the active payment provider**

* Status: Active
* Decision: Whop is the payment provider for premium pass access. Stripe is NOT used.
* Reason: Whop was selected for speed-to-market and plan flexibility (weekly/monthly recurring).
* Do not change without: Explicit founder decision.
* Related implementation impact: Checkout, webhooks, entitlement use Whop APIs. Do not introduce Stripe without approval.

### **DECISION: Magic-link is the auth mechanism for premium access**

* Status: Active
* Decision: Premium session is granted via magic-link email verification, not full OAuth/Google Auth.
* Reason: Fast implementation, no OAuth provider dependency, appropriate for PreMVP.
* Do not change without: Explicit founder approval.
* Related implementation impact: `app/api/auth/magic-link/` + `lib/auth/premiumSession.ts`

### **DECISION: Premium session is cookie-based**

* Status: Active
* Decision: After verified entitlement, a secure server-set cookie grants `/premium` access.
* Reason: Simple, stateless session without full auth provider.
* Do not change without: Architecture decision.
* Related implementation impact: `lib/auth/premiumSession.ts`

---

## 8. Signal Tracking / Proof of Results Decisions (Added 2026-05-21)

### **DECISION: Signal outcomes are tracked, not claimed**

* Status: Active
* Decision: Signal resolver records actual market resolution (YES/NO/VOID) against predicted outcome. This is factual tracking, not a win-rate claim.
* Reason: "Tracked" is honest; "97% win rate" is not.
* Do not change without: Explicit founder approval.
* Related implementation impact: `lib/feed/resolveSignalOutcome.ts`, `scripts/resolve-signals.ts`

### **DECISION: Proof of Results card must not claim win rate or profit**

* Status: Locked
* Decision: The Proof of Results card shows tracked/verified language only. No percentage win-rate, no dollar profit, no ROI.
* Reason: Without verified data at scale, numeric performance claims are misleading.
* Do not change without: Explicit verified data source and founder approval.
* Related implementation impact: Claude Design brief `06_PROOF_OF_RESULTS_CARD_BRIEF.md` and `08_CLAUDE_CODE_HANDOFF_AFTER_DESIGN.md` enforce this.

---

## 9. Claude Design Workflow Decisions (Added 2026-05-21)

### **DECISION: Claude Design is used for visual exploration only**

* Status: Active
* Decision: Claude Design output is visual reference, not source of truth. Source files are source of truth.
* Reason: Design explorations may not match production constraints exactly.
* Do not change without: Founder approval.
* Related implementation impact: Do not copy-paste CSS from Claude Design directly into source files. Claude Code implements from annotated brief, not from raw design output.

### **DECISION: Design pack lives in docs/design/claude-design-source-pack/**

* Status: Active
* Decision: The canonical Claude Design source pack is at `docs/design/claude-design-source-pack/`. Upload ZIP is at `docs/design/polypropicks-claude-design-source-pack-2026-05-20.zip`.
* Reason: Centralized, version-stamped, untracked (intentionally not committed).
* Related implementation impact: Refresh this pack before each Claude Design session if source files changed significantly.
