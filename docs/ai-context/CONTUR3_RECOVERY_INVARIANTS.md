...
5. Генерируемые отчёты не должны пачкать отслеживаемое git-дерево
   (целевое место — git-ignored каталог: `var/reports/contur3/`, override —
   `CONTUR3_REPORT_DIR`; отслеживаемый `reports/contur3/*` — исторический/устаревший
   артефакт, не свежая улика, если явно не сгенерирован туда через override).
