...
## **18. PreMVP14 — Small-task prompt-loop failure (2026-07-29)**

### **18.1 Evidence status**

The executor report is internally consistent and names concrete artifacts, including commit hashes, PR numbers, and deployment status. However, an executor summary is not the highest source of truth.

Until verified directly through current Git/source/runtime evidence, classify the artifact claims as:

```text
SUPPORTED — not yet PROVEN
```

Minimum direct verification:

```cmd
cd /d C:\WORK\KalshiProPulse\sipropicks-premvp1-1
git log --oneline --decorate -20
git show --stat --oneline 4eed14d
git log --oneline --all --since="2026-07-28"
```

Decision table:

```text
A. Only the monitor correction and unconnected classifiers are present
   → executor self-report confirmed; workflow-failure rules remain active.

B. More substantive changes exist than reported
   → reporting understated the work; verify exactly what was deployed.

C. Claimed commits/PR artifacts are absent
   → evidence failure; re-audit the entire period before further implementation.
```

### **18.2 What failed**

Approximately thirty Founder/executor interactions were consumed by a small monitor defect and successive review additions.

The executor-reported shipped value was limited mainly to:

- recognizing an existing queue row by immutable `reservation_id` even when its event time was wrong;
- distinguishing linked-query failure from a genuinely missing queue row;
- proving that out-of-window linkage evidence does not inflate window aggregates.

A separate provider-provenance classifier commit was reported, but it was not connected to the live production path.

The monitor work did not itself advance:

```text
Reservation → queue → Ireland claim → venue order → callback → terminal state
```

### **18.3 Structural analogy with the identity incident**

| Identity defect | Operator-cost defect |
|---|---|
| Each stage rediscovers the entity instead of receiving one immutable ID | Each cycle redefines the task instead of receiving one immutable DoD |
| No single owner of identity | No single owner of “done” |
| Fallback branches keep growing | Review keeps adding one more requirement |
| The entity is eventually lost | The task eventually loses its boundary |

Both incidents share one root disease: no immutable contract fixed once at the start.

### **18.4 Expansion mechanism**

```text
vague task
→ prompt checks one narrow gate
→ review finds one missing check
→ missing check becomes a new task
→ new prompt and new Founder action
→ repeat
```

The critical failure point was the automatic absorption of every new review finding into the current task.

A review finding outside the frozen DoD must be deferred. If it truly blocks DoD, the task must STOP and be formally re-contracted by the Founder.

### **18.5 Why previous rules did not prevent it**
...
They did not control total operator cost.

Missing controls were:

- no operator-cycle budget;
- no immutable TASK CONTRACT;
- no owner of the DoD;
- no global progress counter;
- no automatic stop after repeated non-progress cycles;
- no requirement to disclose whether the task advances live contour or only observability.

Therefore every local step could be technically legal while the overall workflow was economically unacceptable.

### **18.6 Mandatory replacement workflow**

For a small bounded task:
...
Required before the first Founder command:
...
### **18.7 Permanent rules**

1. Default Founder budget: 3 cycles; maximum 5 without explicit re-contract.
2. Every Founder-action response begins with `OPERATOR CYCLE: k / N`.
3. All commands for a cycle are batched into one block.
4. Every command has a pre-declared decision table.
5. Review checklist is fixed before implementation.
6. New ordinary findings go to `DEFERRED FINDINGS`.
7. One correction plus one delta re-review is the maximum normal review loop.
8. Two consecutive non-progress cycles trigger automatic STOP.
9. Measurement, runtime, and product defects are classified separately.
10. Query failure is never interpreted as missing business data.
11. Commit/push is never presented as production effect.
12. Observability and hygiene work are never counted as live-contour progress.
13. Service state is never asserted or changed without exact fresh evidence, timestamp, source, and Founder authorization.
14. Exact service/job/table/file names are verified, never guessed.
15. When the full current context file is unavailable, request it from the Founder and return a complete corrected replacement instead of telling another agent to open and reconcile it.

### **18.8 Necessary nuance**

The protocol must not become another rigid source of delay.

- The TASK CONTRACT is immutable during execution, but may be replaced through explicit Founder-approved re-contract after STOP.
- A reviewer may block on a critical correctness/security/money/data-integrity defect inside the changed diff even when the original checklist missed it. The finding must be marked `CHECKLIST_GAP`; it cannot be silently fixed through scope expansion.
- The “rule of two” treats fresh decision-changing evidence as progress for inspect-only tasks. It requires repository artifacts for implementation tasks.
- Numeric estimates are forbidden before proof and scope freeze. After evidence exists, bounded planning estimates are allowed only with explicit gates, assumptions, and confidence.

### **18.9 Role failure**

In both the identity incident and this operator-cost incident, the Founder detected the global invariant failure before the architect/executor stopped itself.

This is a technical-lead role failure.

The architect/executor must now own the global stop condition. If the Founder has to notice the loop first, the incident must be logged as another process failure.
