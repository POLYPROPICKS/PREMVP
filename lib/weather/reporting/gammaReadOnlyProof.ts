...
type Trace = { stage: string; input_count: number; output_count: number; first_rejection_reason: string | null; target_present: boolean; elapsed_ms: number };
...
export async function runGammaReadOnlyProof(options: { now?: () => string; stationIds?: string[] } = {}): Promise<GammaReadOnlyProof> {
  const fetched = await fetchGammaWeatherPage(); const traces: Trace[] = [{ stage: "fetch", input_count: 1, output_count: 1, first_rejection_reason: null, target_present: true, elapsed_ms: fetched.elapsedMs }];
  const validatorStarted = Date.now(); let parsed: unknown; try { parsed = JSON.parse(fetched.rawText); } catch { throw new Error("GAMMA_PROOF_VALIDATION:INVALID_JSON"); }
...
export function buildGammaProofReport(proof: GammaReadOnlyProof) {
  const report = { request_timestamp: proof.requestTimestamp, endpoint: proof.endpoint, http_status: proof.httpStatus, response_bytes: proof.responseBytes, response_sha256: proof.responseSha256, envelope_type: proof.envelopeType, raw_market_count: proof.snapshot.rawMarkets, accepted_market_count: proof.snapshot.acceptedMarkets, contract_count: proof.snapshot.contracts, attribution_counts: proof.snapshot.attributionCounts, sample_condition_ids: proof.snapshot.sampleConditionIds, sample_token_ids: proof.snapshot.sampleTokenIds, stage_trace: proof.traces, database_writes: proof.databaseWrites, supabase_access: proof.supabaseAccess, limitations: proof.limitations };
  const markdown = `# Weather Gamma Read-only Proof\n\n- HTTP status: ${report.http_status}\n- Response bytes: ${report.response_bytes}\n- Response SHA-256: ${report.response_sha256}\n- Raw/accepted/contracts: ${report.raw_market_count}/${report.accepted_market_count}/${report.contract_count}\n- gamma_network_proven: ${proof.limitations.gamma_network_proven}\n- gamma_envelope_proven: ${proof.limitations.gamma_envelope_proven}\n- condition_identity_proven: ${proof.limitations.condition_identity_proven}\n- token_identity_proven: ${proof.limitations.token_identity_proven}\n- postgres_proven: NO\n- migration_applied: NO\n- database_write_proven: NO\n- runtime_proven: NO\n`;
  return { json: JSON.stringify(report, null, 2), markdown };
}
