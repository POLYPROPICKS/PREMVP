...
export type InventoryMarket = { conditionId: string; venueEventId: string | null; venueMarketId: string | null; title: string | null; slug: string | null; active: boolean; closed: boolean; outcomes: Array<{ tokenId: string; outcome: string; outcomeIndex: number }>; attributionStatus: AttributionStatus; attributionReason: string | null };
...
export function extractWeatherMarkets(page: RawPage): { markets: InventoryMarket[]; trace: InventoryTrace } {
  ...
  for (const event of page.records) for (const marketRaw of values(event.markets)) {
    ...
    const rawOutcomes = values(market.outcomes); const tokenIds = values(market.clobTokenIds ?? market.clob_token_ids);
    if (!conditionId) { first ||= "MISSING_CONDITION_ID"; continue; }
    if (!rawOutcomes.length || rawOutcomes.length !== tokenIds.length || !tokenIds.every(text)) { first ||= "MALFORMED_OUTCOMES_OR_TOKENS"; continue; }
    const outcomes = rawOutcomes.map((outcome, index) => ({ tokenId: text(tokenIds[index])!, outcome: typeof outcome === "string" ? outcome : text(record(outcome).name) ?? "", outcomeIndex: index }));
