...
export interface ScientificExecutionCandidate {
  observationId: string; decisionAtIso: string; resolvedAtIso: string;
  finalScore: number; dataCoverage: number; entryPrice: number; row: unknown;
}
...
export interface ScientificCapitalReplay {
  ...
  settled: number; open: number; endingFreeActive: number; endingOpenPrincipal: number; endingActive: number; endingVault: number; endingTotal: number; minimumTotal: number;
...
export function replayScientificCapitalPolicy(candidates: readonly ScientificExecutionCandidate[], policy: ScientificCapitalPolicy, capacity: ScientificCapacity, options: { finalizationMode?: "SETTLE_ALL" | "KEEP_OPEN_AT_CUTOFF"; cutoffAtIso?: string } = {}): ScientificCapitalReplay {
  ...
  const finalizationMode = options.finalizationMode ?? "SETTLE_ALL";
  const cutoff = finalizationMode === "KEEP_OPEN_AT_CUTOFF" ? Date.parse(options.cutoffAtIso ?? "") : Infinity;
  if (finalizationMode === "KEEP_OPEN_AT_CUTOFF" && !Number.isFinite(cutoff)) throw new Error("cutoffAtIso is required for KEEP_OPEN_AT_CUTOFF");
  ...
  let maxActiveFall = 0;
  let invalid = 0;
  const open: Array<{ candidate: ScientificExecutionCandidate; resolved: number; stake: number }> = [];
  ...
  for (let index = 0; index < ordered.length;) {
    ...
    settle(at);
    const batch: ScientificExecutionCandidate[] = [];
  ...
  settle(cutoff);
  point(finalizationMode === "SETTLE_ALL" ? "FINAL" : new Date(cutoff).toISOString());
  ...
  return { version: SCIENTIFIC_CAPITAL_ARCHITECTURE_VERSION, policy, capacity: { ...capacity, maxAcceptedPerOperatingDay: dailyLimit }, eligibleMatches: candidates.length, executedMatches: executed.length, settled: executed.length - open.length, open: open.length, skippedPositions: candidates.length - executed.length, skipReasons, wins: executed.filter((entry) => entry.result === "win").length, losses: executed.filter((entry) => entry.result === "loss").length, totalStaked, netPnl, roi: totalStaked ? round(netPnl / totalStaked * 100) : null, endingFreeActive: last.freeActive, endingOpenPrincipal: last.openPrincipal, endingActive: last.active, endingVault: last.vault, endingTotal: last.total, minimumTotal: round(minimumTotal), maximumFallFromTotalPeak: round(maxTotalFall), maximumFallFromActivePeak: round(maxActiveFall), invalidCapitalStates: invalid, blockPnl, ledger, curve, transfers, executionLedgerHash: stableHash(ledger), capitalCurveHash: stableHash(curve) };
}
