...
// Status words that, on their own, constitute authoritative fill/terminal
// evidence for an already-placed CLOB order. Deliberately excludes
// REJECTED_ORDER_STATUSES -- a rejection is handled by the separate
// classifyOrderEvent/markQueueTerminalFromOrderEvent path below, not as an
// order-event progression.
const PROGRESSION_EVIDENCE_STATUSES = new Set([
  "matched",
  "filled",
  "fully_filled",
  "partially_filled",
...
// Fields that, when present, are themselves authoritative fill evidence
// regardless of the status string used.
const PROGRESSION_EVIDENCE_FIELDS = [
  "executed_size",
  "filled_size",
  "filled_price",
  "average_fill_price",
...
/**
 * A changed economic payload under the same idempotency_key is only ever a
 * legitimate progression when the callback itself carries explicit fill/
 * terminal evidence -- an authoritative matched/filled/partial/unfilled/
 * expired status, or an explicit fill field. Merely differing price/size
 * with no such evidence is never enough (it cannot be told apart from a
 * client resending a mutated request under a stale idempotency_key), and
 * stays a strict conflict.
 */
function hasExplicitProgressionEvidence(raw: Record<string, unknown>): boolean {
 ...
 *    condition_id, token_id, side, clob_order_id), clob_order_id is
 *    present, AND the callback carries explicit fill/terminal evidence
 *    (see hasExplicitProgressionEvidence) -- this is a legitimate
 *    terminal/fill update of the same already-accepted CLOB order (e.g.
 *    actual fill price/size differs from the originally requested
 *    price/size). Persist the progression facts onto the existing row
 *    (never the originally requested submitted_price/submitted_size,
 *    which remain immutable); never insert a second row.
 *  - CONFLICT: immutable identity does not match (wrong condition_id/
 *    token_id/side/clob_order_id), the order has no clob_order_id yet to
 *    anchor a progression to, or there is no explicit fill/terminal
 *    evidence for the changed economic payload -- fail closed, same as
 *    the original strict duplicate/conflict protection.
 */
...
export async function handleOrderEventSubmission(
  ...
  if (existingByIdempotency) {
    const classification = classifyOrderEventAgainstExisting(canonical, canonicalFromStoredEvent(existingByIdempotency), raw);
  ...
  if (insertResult.code === "UNIQUE_VIOLATION_IDEMPOTENCY_KEY") {
    ...
    const classification = classifyOrderEventAgainstExisting(canonical, canonicalFromStoredEvent(canonicalRow), raw);
    if (classification === "CONFLICT") return { kind: "CONFLICT_IDEMPOTENCY" };
    if (classification === "PROGRESSION") {
